...
- (void)move {
    ...
    NSLog(@"path = %@", path);
    NSBundle *testToolBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[ZZTestBaseViewController class]] pathForResource:@"TestBundle" ofType:@"bundle"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([ViewController_Plist isEqualToString:title]) {
        ...
        [self.navigationController pushViewController:[[ZZConfigBrowerPage alloc] initWithObjectsDictionary:@{
            @"system Dict" : array1,
            @"file Dict" : array2,
            @"cache Dict" : array3,
            @"whatever" : array4,
