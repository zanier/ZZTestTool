...
@implementation ZZPlistBrowerNode
...
- (void)setValue:(id)value {
    ...
    if ([value isKindOfClass:[NSArray class]]) {
        ...
        for (id child in array) {
            ZZPlistBrowerNode *node = [[ZZPlistBrowerNode alloc] initWithKey:nil value:child depth:self.depth + 1];
        ...
        for (id key in dictionary.allKeys) {
            ...
            ZZPlistBrowerNode *node = [[ZZPlistBrowerNode alloc] initWithKey:key value:value depth:self.depth + 1];
            [self.children addObject:node];
        }
    }
}
...
- (void)setDepth:(NSInteger)depth {
    ...
    _depth = depth;
    for (ZZPlistBrowerNode *node in self.children) {
...
- (NSMutableArray<ZZPlistBrowerNode *> *)visiableChildren {
    ...
    for (ZZPlistBrowerNode *child in self.children) {
        if (child.isVisiable) {
...
- (NSMutableArray<ZZPlistBrowerNode *> *)children {
    if (!_children) {
...
- (id)copyWithZone:(NSZone *)zone {
    ZZPlistBrowerNode *copyItem = [[ZZPlistBrowerNode allocWithZone:zone] init];
    if (copyItem) {
