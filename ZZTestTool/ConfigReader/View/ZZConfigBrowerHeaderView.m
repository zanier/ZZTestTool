...
#import "ZZConfigBrowerHeaderView.h"

static CGFloat const ZZConfigBrowerHeaderViewHeight = 49.0f;

@implementation ZZConfigBrowerHeaderItem

@end

@interface ZZConfigBrowerHeaderView ()

@property (nonatomic, strong) UIScrollView *scrollView;

@property (nonatomic, strong) NSArray<ZZConfigBrowerHeaderItem *> *items;

@property (nonatomic, strong) NSMutableArray<UIButton *> *buttonArray;

@end

@implementation ZZConfigBrowerHeaderView

/// MARK: - init

+ (instancetype)viewWithextArray:(NSArray<NSString *> *)textArray {
    CGFloat width = [UIScreen mainScreen].bounds.size.width;
    ZZConfigBrowerHeaderView *view = [[ZZConfigBrowerHeaderView alloc] initWithFrame:CGRectMake(0, 0, width, ZZConfigBrowerHeaderViewHeight) textArray:textArray];
    return view;
}
...
- (void)setTextArray:(NSArray<NSString *> *)textArray {
    ...
    NSMutableArray<ZZConfigBrowerHeaderItem *> *items = [NSMutableArray arrayWithCapacity:textArray.count];
    [textArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ZZConfigBrowerHeaderItem *item = [[ZZConfigBrowerHeaderItem alloc] init];
        item.text = obj;
...
- (void)setItems:(NSArray<ZZConfigBrowerHeaderItem *> *)items {
...
- (void)setupSubView {
    ...
    self.scrollView.contentSize = CGSizeMake(left, ZZConfigBrowerHeaderViewHeight);
}
...
+ (CGFloat)defaultHeight {
    return ZZConfigBrowerHeaderViewHeight;
}

- (UIButton *)buttonWithItem:(ZZConfigBrowerHeaderItem *)item {
    ...
    CGSize fitedSize = [button sizeThatFits:CGSizeMake([UIScreen mainScreen].bounds.size.width, ZZConfigBrowerHeaderViewHeight)];
