...
#import "ZZFileBrowerController.h"
...
@interface ZZFileBrowerController () <ZZFileBrowerScrollHeaderDelegate, ZZFileBrowerPageDelegate, UINavigationControllerDelegate, ZZQLPreviewControllerDataSource> {
    ZZFileBrowerItem *_currentItem;
}

@property (nonatomic, strong) UINavigationController *navigation;
@property (nonatomic, strong) ZZFileBrowerPage *rootPage;

@property (nonatomic, strong) UIToolbar *headerToolBar;
@property (nonatomic, strong) ZZFileBrowerScrollHeader *scrollHeader; // 顶部滑动式图

@property (nonatomic, strong) NSMutableArray<ZZFileBrowerItem *> *pathNavigation;

@end

@implementation ZZFileBrowerController

/// MARK: - init

+ (instancetype)createPage:(NSDictionary *)params {
    NSString *rootPath = nil;
    if (params && params[ZZFileBrowerControllerRootPathKey]) {
        rootPath = params[ZZFileBrowerControllerRootPathKey];
    }
    ZZFileBrowerController *page = [[ZZFileBrowerController alloc] initWithRootPath:rootPath];
    return page;
}
...
- (void)loadRootPath:(NSString *)rootPath {
    ...
    if (error) return;
    _currentItem = [[ZZFileBrowerItem alloc] initWithPath:rootPath];
    _currentItem.attributes = attributes;
    // root page
    _rootPage = [[ZZFileBrowerPage alloc] init];
    ...
    // 移动到当前目录
    UIImage *image = [ZZFileBrowerManager imageWithFileType:_currentItem.type scale:1];
...
/// @param item 文件夹数据
- (BOOL)enterDirectoryWithItem:(ZZFileBrowerItem *)item {
    BOOL didReload = NO;
    _currentItem = item;
    if ([self.pathNavigation containsObject:item]) {
        ...
        // pop
        ZZFileBrowerPage *page = _navigation.viewControllers[idx];
        ...
        if (nextPage) {
            ...
            /// 移动到当前目录
            UIImage *image = [ZZFileBrowerManager imageWithFileType:item.type scale:1];
...
- (UIViewController *)pageWithItem:(ZZFileBrowerItem *)item {
    if (item.isDir) {
        ZZFileBrowerPage *filePage = [[ZZFileBrowerPage alloc] init];
        filePage.delegate = self;
        [filePage reloadAtDirectoryWithItem:item];
        return filePage;
    } else if (item.type == ZZFileBrowerFileTypePList) {
        Class plistBrowerPagCls = NSClassFromString(@"ZZPlistBrowerPage");
...
- (void)viewWillLayoutSubviews {
    ...
    frame.origin.y = ZZFileBrower_NavBarBottom - topPoint.y;
...
- (ZZFileBrowerScrollHeader *)scrollHeader {
    if (!_scrollHeader) {
        _scrollHeader = [ZZFileBrowerScrollHeader viewWithextArray:nil imageArray:nil];
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    BOOL isFileBrowerPage = [viewController isKindOfClass:[ZZFileBrowerPage class]];
...
/// @param idx 选择级别索引
- (BOOL)scrollHeader:(ZZFileBrowerScrollHeader *)scrollHeader shouldSelectAtIndex:(NSInteger)idx {
    return YES;
}
...
/// @param scrollHeader 滑动选择视图
/// @param idx 选择级别索引
- (void)scrollHeader:(ZZFileBrowerScrollHeader *)scrollHeader didSelectAtIndex:(NSInteger)idx {
    /// 进入目标文件夹
    ZZFileBrowerItem *item = self.pathNavigation[idx];
    [self enterDirectoryWithItem:item];
}

/// MARK: - <ZZFileBrowerScrollHeaderDelegate>
