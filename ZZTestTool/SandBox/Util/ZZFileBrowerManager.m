...
@implementation ZZFileBrowerManager

static ZZFileBrowerManager *manager;
static dispatch_once_t onceToken;

+ (instancetype)manager {
    dispatch_once(&onceToken, ^{
        manager = [[ZZFileBrowerManager alloc] init];
    });
    return manager;
}
...
static NSString *const ZZFileBrowerErrorDomin = @"ZZFileBrowerErrorDomin";

+ (NSError *)error_invalidFilePath:(NSString *)path {
    NSString *desc = [NSString stringWithFormat:@"Invalid file path: %@", path];
    return [NSError errorWithDomain:ZZFileBrowerErrorDomin code:-1 userInfo:@{
...
+ (NSArray<ZZFileBrowerItem *> *)contentItemsOfItem:(ZZFileBrowerItem *)parent {
    ...
    for (NSString *content in contents) {
        ...
        ZZFileBrowerItem *child = [[ZZFileBrowerItem alloc] initWithPath:childPath];
...
+ (void)dealWithPath:(NSString *)path {
    [ZZFileBrowerManager manager].dealtPath = path;
}
...
//+ (void)removeItem:(ZZFileBrowerItem *)item error:(NSError **)error {
//    [self removeItemAtPath:item.path error:error];
//}
//
//+ (void)renameItem:(ZZFileBrowerItem *)item name:(NSString *)name error:(NSError **)error {
//    [self renameItemAtPath:item.path name:name error:error];
//}

//+ (void)copyItem:(ZZFileBrowerItem *)item {
...
+ (NSString *)stringFromDateByDateAndTime:(NSDate *)date {
    [ZZFileBrowerManager.manager.dateFormatter setDateFormat:@"YYYY年MM月DD日 mm:ss"];
    return [ZZFileBrowerManager.manager.dateFormatter stringFromDate:date];
}

+ (NSString *)stringFromDateByDate:(NSDate *)date {
    [ZZFileBrowerManager.manager.dateFormatter setDateFormat:@"YYYY年MM月DD日"];
    return [ZZFileBrowerManager.manager.dateFormatter stringFromDate:date];
}

+ (NSString *)stringFromDateByTime:(NSDate *)date {
    [ZZFileBrowerManager.manager.dateFormatter setDateFormat:@"mm:ss"];
    return [ZZFileBrowerManager.manager.dateFormatter stringFromDate:date];
}
...
+ (NSString *)duplicateNameWithOriginName:(NSString *)name amongItems:(NSArray<ZZFileBrowerItem *> *)items {
    ...
    for (int i = 0; i < INT32_MAX; i++) {
        ...
        BOOL exist = NO;
        for (ZZFileBrowerItem *child in items) {
...
//+ (ZZFileBrowerFileType)fileTypeWithExtension:(NSString *)extension;
//+ (NSString *)imageNameWithType:(ZZFileBrowerFileType)type;
//+ (UIImage *)imageNameWithItem:(ZZFileBrowerItem *)item;

+ (UIImage *)imageWithFileType:(ZZFileBrowerFileType)type scale:(NSInteger)scale {
    NSString *imageName = ZZImageNameWithType(type);
    switch (scale) {
        case 1: break;
...
+ (UIImage *)imageWithFileType:(ZZFileBrowerFileType)type {
    NSString *imageName = ZZImageNameWithType(type);
...
+ (UIImage *)imageWithActionText:(NSString *)text {
    NSString *imageName = ZZActionImageNameWithActionText(text);
